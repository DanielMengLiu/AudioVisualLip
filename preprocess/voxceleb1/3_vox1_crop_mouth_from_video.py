...
def load_args(default_config=None):
    ...
    # -- utils
    parser.add_argument('--video-dir', default='/datasets2/voxceleb1/video/test/', help='video directory')
    parser.add_argument('--lip-dir', default='/data/linchenghan/lip_inter_dataset/voxceleb1/lip/', help='the directory of saving mouth ROIs')
    parser.add_argument('--landmark-dir', default='/datasets2/voxceleb1/landmark/test/', help='landmark directory')
    parser.add_argument('--manifest', default='../../data/trial/vox1_O-29.txt', help='list of detected video and its subject ID')
    parser.add_argument('--failure-file',
                        default='/data/linchenghan/lip_inter_dataset/voxceleb1/lip/vox1_lip.failure',
                        help='failure description file')

    # -- mean face utils
    parser.add_argument('--mean-face', default='/data/linchenghan/lip_inter_dataset/voxceleb1/vox1_mean_face_30000.npy',
...
def run(files,failure_list):
    for filename in tqdm(files):
        video_path = os.path.join(args.video_dir, filename+'.mov')
...
if(__name__ == '__main__'):
    ...
    data = list(set([x.split(' ')[1] for x in lines]).union([x.split(' ')[2] for x in lines]))

...
def load_args(default_config=None):
    ...
    parser.add_argument('--manifest', default='/data/liumeng/SyncLip2/data/manifest/grid_test_manifest.csv', help='the manifest for landmark detection') #/data/liumeng/SyncLip2/preprocess/lrs3/lrs3_landmark.failure
...
def run(gpu, iStart, iEnd, files, v_list, queue=0):
    ...
    for fl in range(iStart,iEnd):
        file = os.path.join(args.video_dir, files[fl]+'.mpg')
...
def extract_lip(manifest):
    ...
    if 'failure' not in manifest:
        with open('./grid/grid_landmark.failure','a+') as w:

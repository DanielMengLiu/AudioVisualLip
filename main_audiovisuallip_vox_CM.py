...
with open('./conf/savedsolution_audiovisuallip_vox_CM.yaml', 'r') as f:
    OPTS = yaml.load(f, Loader=CLoader)
# shutil.copyfile('./conf/config_audiovisuallip_lrs3.yaml', './conf/savedsolution_audiovisuallip_lrs3_CM.yaml')
...
class Trainer(object):
    def __init__(self):
        ...
        param_groups = [{'params': self.audiomodel.parameters(), 'lr': 0.0001}, 
                        {'params': self.visualmodel.parameters(), 'lr': 0.0001}, 
        ...
        if self.stageopts['lr_scheduler'] == 'steplr':
            ...
            self.lr_scheduler = optim.lr_scheduler.MultiStepLR(self.optim, milestones=[2,3], gamma=0.1)
            # self.lr_scheduler = optim.lr_scheduler.MultiStepLR(self.optim, milestones=[25,50], gamma=0.1)
        ...
        if self.audioresume == 'None' or self.visualresume == 'None':
            self.exp = '_'.join(['lrs3', 'cross-modal', self.audiomodelname, self.visualmodelname, '_3aug4blocks'])
        else:
            self.exp = self.audioresume.split('/')[1]

        # continue training
        if (self.audioresume != 'None' and os.path.exists(self.audioresume)) or (self.visualresume != 'None' and os.path.exists(self.visualresume)) or (self.audiovisualresume != 'None' and os.path.exists(self.audiovisualresume)):
            self._load()
        else:
    ...
    def _train_epoch(self):
        ...
        for batch_idx, (audiofeats, videofeats, targets_label) in enumerate(progress_bar):
            ...
            avloss = 0.25*aloss + 0.25*vloss + 0.25*traloss + 0.25*trvloss
            avloss.backward()

...
with open('./conf/savedsolution_audio_lrs3_IT.yaml', 'r') as f:
...
class Trainer(object):
    def __init__(self):
        ...
        if self.resume == 'None':
            ...
            self.exp = '_'.join(['lrs3', 'independent-training', self.modelname, 'noisespec3blocks'])
        else:
    ...
    def _score_embedding(self, stage, trials, score_dict):
        ...
        emb_dir = os.path.join('exp/{}/{}_emb'.format(self.exp, stage))
        for line in trials:
            embedding_11 = torch.FloatTensor(np.load(os.path.join(emb_dir, line.split()[-2]+'.npz'), allow_pickle=True)['arr_0'][0])
            embedding_12 = torch.FloatTensor(np.load(os.path.join(emb_dir, line.split()[-2]+'.npz'), allow_pickle=True)['arr_0'][1])
            embedding_21 = torch.FloatTensor(np.load(os.path.join(emb_dir, line.split()[-1]+'.npz'), allow_pickle=True)['arr_0'][0])
            embedding_22 = torch.FloatTensor(np.load(os.path.join(emb_dir, line.split()[-1]+'.npz'), allow_pickle=True)['arr_0'][1])
...
if __name__ == '__main__':
    ...
    # voxceleb dataset test
        ...
        if stage in ['test', 'cohort']:
            tester._eval_network(stage=stage, num_workers=6)
        if stage in ['submean']:
